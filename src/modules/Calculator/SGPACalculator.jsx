...
export default function SGPACalculator() {
    ...
    const getSGPAConditionalColor = (val) => {
        if (val >= 9.0) return 'text-[#7B61FF] drop-shadow-[0_0_20px_#7B61FF]';
    ...
    return (
        <div className="w-full flex flex-col items-center gap-8 py-8 px-4 md:px-8">
            {/* Live SGPA Badge */}
            <div className="flex flex-col items-center justify-center p-8 bg-[#12121A] rounded-[2rem] border border-white/5 shadow-2xl min-w-[300px]">
                <span className="text-[#F0EFF4]/60 font-mono text-sm tracking-widest mb-2 uppercase">Live SGPA</span>
                <div
                    id="sgpa-badge"
                    ref={sgpaRef}
            ...
            <div id="print-transcript" className="w-full max-w-6xl">
                <div className="flex justify-between items-end mb-6">
                    <div>
                        ...
                        <p className="font-mono text-[#F0EFF4]/50 mt-1 flex items-center gap-4">
                            Cumulative CGPA: <span className="text-white text-xl font-bold ml-1">{currentCGPA.toFixed(2)}</span>
                        </p>
                    </div>
                    <button
                        onClick={handleExportPDF}
                        className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 text-[#F0EFF4] font-mono text-sm rounded-full transition-colors"
                    >
                ...
                <div className="flex overflow-x-auto gap-4 pb-4 snap-x">
                  ...
                  ${activeSemesterId === sem.id ? 'bg-[#7B61FF]/10 border-[#7B61FF]/50' : 'bg-[#12121A] border-white/5'}`}
                            >
                                ...
                                <div className="flex justify-between text-[#F0EFF4]/50 font-mono text-xs">
